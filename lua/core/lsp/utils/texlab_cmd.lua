...
CMD.cmd_backsearch = function(file, line, CR_char)
    local cmd = "nvim --server /tmp/nvim.latex --remote-send " .. string.format("\":lua DROP_TEX(%s,%s)%s\"", file, line, CR_char)
...
CMD.zathura = {
    executable = "zathura",
    args = { "-x", CMD.cmd_backsearch("'%{input}'", "%{line}", "<CR>"),
        "--synctex-forward", "%l:1:%f", "%p" }
}

CMD.sioyek = {
    executable = "sioyek",
    args = { "--inverse-search", CMD.cmd_backsearch("'%1'", "%2", "\r"), -- %1 file, %2 line
